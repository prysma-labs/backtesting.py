...
class Strategy(metaclass=ABCMeta):
    ...
    def __str__(self):
        params = ",".join(
            f"{i[0]}={i[1]}"
            for i in zip(
                self._params.keys(), map(_as_str, self._params.values()), strict=False
            )
        )
        if params:
            params = "(" + params + ")"
    ...
    def I(  # noqa: E743
        ...
        **kwargs,
        ...
        elif try_(lambda: all(isinstance(item, str) for item in name), False):  # type: ignore[union-attr]
        ...
        if value is not None:
            value = try_(lambda: np.asarray(value, order="C"), None)  # type: ignore[arg-type]
        is_arraylike = bool(value is not None and value.shape)  # type: ignore[union-attr]

        # Optionally flip the array if the user returned e.g. `df.values`
        if is_arraylike and np.argmax(value.shape) == 0:  # type: ignore[union-attr]
            value = value.T  # type: ignore[union-attr]

        if isinstance(name, list) and (np.atleast_2d(value).shape[0] != len(name)):  # type: ignore[arg-type]
            raise ValueError(
                f"Length of `name=` ({len(name)}) must agree with the number "
                f"of arrays the indicator returns ({value.shape[0]})."  # type: ignore[union-attr]
            )

        if (
            not is_arraylike
            or not 1 <= value.ndim <= 2  # type: ignore[union-attr]
            or value.shape[-1] != len(self._data.Close)  # type: ignore[union-attr]
        ):
            raise ValueError(
        ...
        if overlay is None and np.issubdtype(value.dtype, np.number):  # type: ignore[union-attr]
...
class Order:
    ...
    def __repr__(self):
        return "<Order {}>".format(
            ", ".join(
                f"{param}={try_(lambda v=value: round(v, 5), value)!r}"  # type: ignore[arg-type]
                for param, value in (
                    ("size", self.__size),
...
class Trade:
    ...
    def entry_time(self) -> Union[pd.Timestamp, int]:
        ...
        return self.__broker._data.index[self.__entry_bar]  # type: ignore[return-value]

    @property
    def exit_time(self) -> Union[pd.Timestamp, int] | None:
        """Datetime of when the trade was exited."""
        if self.__exit_bar is None:
            return None
        return self.__broker._data.index[self.__exit_bar]  # type: ignore[return-value]
...
class _Broker:
    def __init__(
        ...
        *,
        ...
        if callable(commission):
            self._commission = commission  # type: ignore[assignment]
        else:
            try:
    ...
    def _adjusted_price(self, size=None, price=None) -> float:
        ...
        """
        return (price or self.last_price) * (1 + copysign(self._spread, size or 0))
            ...
            if -1 < size < 1:
                ...
                if not size:
                    warnings.warn(
                        ...
                        f"insufficient margin (equity={self.equity:.2f}, "
    ...
    def _open_trade(
        self, price: float, size: int, sl: float | None, tp: float | None, time_index: int, tag
    ):
...
class Backtest:
    ...
    def __init__(
        ...
        *,
        ...
        if (
            ...
            (
                pd.api.types.is_numeric_dtype(data.index)
                and (data.index > pd.Timestamp("1975").timestamp()).mean() > 0.8  # type: ignore[operator]
            )
        ):
            try:
        ...
        if data[["Open", "High", "Low", "Close"]].isnull().values.any():  # type: ignore[union-attr]
            raise ValueError(
    ...
    def run(self, **kwargs) -> pd.Series:
        ...
        with np.errstate(invalid="ignore"):
            ...
            equity = np.asarray(pd.Series(broker._equity).bfill().fillna(broker._cash).values)
    ...
    def optimize(
        ...
        *,
        ...
        method: str = "grid",
        max_tries: Union[int, float] | None = None,
        ...
        * `"sambo"` which finds close-to-optimal strategy parameters using
        ...
        if isinstance(maximize, str):
            ...
            def _maximize_by_key(stats: pd.Series, _key=maximize) -> float:
                return float(stats[_key])  # type: ignore[arg-type]

            maximize = _maximize_by_key  # type: ignore[assignment]
        ...
        if constraint is None:
            ...
            def _no_constraint(_) -> bool:
                return True

            constraint = _no_constraint
        ...
        def _optimize_grid() -> Union[pd.Series, tuple[pd.Series, pd.Series]]:
            ...
            heatmap = pd.Series(
                ...
                index=pd.MultiIndex.from_tuples(
                    [tuple(p.values()) for p in param_combos],
            ...
            if pd.isnull(heatmap).all():
                ...
                stats = self.run(**dict(zip(heatmap.index.names, best_params, strict=False)))  # type: ignore[arg-type]
        ...
        def _optimize_sambo() -> Union[
            ...
            def memoized_run(tup):
                ...
                return -maximize(stats)  # type: ignore[operator]
            ...
            def objective_function(x) -> float:
                ...
                return 0.0 if np.isnan(value) else float(value)  # type: ignore[arg-type]

            def cons(x):
                nonlocal constraint, _names
                return constraint(AttrDict(zip(_names, x, strict=False)))  # type: ignore[misc]

            res = sambo.minimize(
                fun=objective_function,
                bounds=dimensions,
                constraints=cons,
                max_iter=int(max_tries),
                method="sceua",
                rng=random_state,
            )
            ...
            if return_heatmap:
                heatmap = pd.Series(
                    dict(zip(map(tuple, res.xv), -res.funv, strict=False)), name=maximize_key
                )
    ...
    def plot(
        ...
        *,
        ...
        return plot(
            ...
            superimpose=superimpose,  # type: ignore[arg-type]
            resample=resample,
